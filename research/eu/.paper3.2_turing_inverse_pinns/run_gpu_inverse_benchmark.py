#!/usr/bin/env python3
"""
run_gpu_inverse_benchmark.py
============================
100% GPU-native execution of the Schnakenberg Turing Inverse PINN benchmark:
- Thermally gentle single CPU worker (CPU stays completely cool).
- Forward simulation via GPU spectral splitting (reflect + FFT2 on CUDA) in 17s.
- Inverse PINN on CUDA GPU (RTX 3050).
- Real-time logging of parameter convergence and error metrics.
- Publication-quality figures & tables from actual training data.
"""
...
print("  THERMAL CONTROL: Single-thread CPU interop + 100% CUDA GPU execution")
print("="*82)
...
# -----------------------------------------------------------------------------
# 1. FORWARD SIMULATION (GPU SPECTRAL SPLITTING)
# -----------------------------------------------------------------------------
print("\n[PHASE 1] Forward High-Fidelity Simulation on GPU (Pereira 2019 Benchmark)...")
...
N_sim = 100
x_sim = torch.linspace(0, L, N_sim, device=device)
y_sim = torch.linspace(0, L, N_sim, device=device)
Y_sim, X_sim = torch.meshgrid(y_sim, x_sim, indexing='ij')

# Symmetric extension for zero-flux Neumann BC
def reflect_2d(f):
    f_top = f
    f_bot = torch.flip(f[1:-1, :], dims=[0])
    f_full_y = torch.cat([f_top, f_bot], dim=0)
    f_left = f_full_y
    f_right = torch.flip(f_full_y[:, 1:-1], dims=[1])
    return torch.cat([f_left, f_right], dim=1)

N_ext = 2 * N_sim - 2
kx = torch.fft.fftfreq(N_ext, d=L/(N_sim-1), device=device) * 2 * torch.pi
ky = torch.fft.fftfreq(N_ext, d=L/(N_sim-1), device=device) * 2 * torch.pi
KY, KX = torch.meshgrid(ky, kx, indexing='ij')
Lap_eig = - (KX**2 + KY**2)

# Initial Condition: Eq. 2.31 of Thesis
u_gpu = u_star + 1e-3 * torch.exp(-100.0 * ((X_sim - 1.0/3.0)**2 + (Y_sim - 0.5)**2))
v_gpu = torch.full_like(u_gpu, v_star)

def reaction_rk4(u_c, v_c, dt_sub):
    def f(u_, v_):
        return kappa_true * (a - u_ + (u_**2)*v_), kappa_true * (b - (u_**2)*v_)
    k1u, k1v = f(u_c, v_c)
    k2u, k2v = f(u_c + 0.5*dt_sub*k1u, v_c + 0.5*dt_sub*k1v)
    k3u, k3v = f(u_c + 0.5*dt_sub*k2u, v_c + 0.5*dt_sub*k2v)
    k4u, k4v = f(u_c + dt_sub*k3u, v_c + dt_sub*k3v)
    return u_c + (dt_sub/6.0)*(k1u + 2*k2u + 2*k3u + k4u), v_c + (dt_sub/6.0)*(k1v + 2*k2v + 2*k3v + k4v)

def diffuse_fft(u_c, v_c, dt_sub):
    u_ext = reflect_2d(u_c)
    v_ext = reflect_2d(v_c)
    u_hat = torch.fft.fft2(u_ext) * torch.exp(Du_true * Lap_eig * dt_sub)
    v_hat = torch.fft.fft2(v_ext) * torch.exp(Dv_true * Lap_eig * dt_sub)
    u_diff = torch.real(torch.fft.ifft2(u_hat))[:N_sim, :N_sim]
    v_diff = torch.real(torch.fft.ifft2(v_hat))[:N_sim, :N_sim]
...
for s in range(1, total_steps + 1):
    u_gpu, v_gpu = diffuse_fft(u_gpu, v_gpu, dt * 0.5)
    u_gpu, v_gpu = reaction_rk4(u_gpu, v_gpu, dt)
    u_gpu, v_gpu = diffuse_fft(u_gpu, v_gpu, dt * 0.5)
    t_now += dt
    ...
    if snap_idx < len(target_snaps) and abs(t_now - target_snaps[snap_idx]) < dt * 0.5:
        ...
        print(f"  [GPU Solver] Saved Snapshot t = {ts:.2f} s | u in [{u_gpu.min().item():.3f}, {u_gpu.max().item():.3f}] | v in [{v_gpu.min().item():.3f}, {v_gpu.max().item():.3f}]")
        snap_idx += 1

torch.cuda.synchronize()
print(f"  --> Forward simulation finished in {time.time()-t_start_fwd:.2f} s on GPU.")

# Save npz
np.savez(
    os.path.join(out_dir, "exact_thesis_turing_solution.npz"),
    x=x_sim.cpu().numpy(), y=y_sim.cpu().numpy(), t_eval=np.array(target_snaps),
    **snaps_u, **snaps_v
)

# -----------------------------------------------------------------------------
# 2. INVERSE PINN (100% CUDA)
# -----------------------------------------------------------------------------
...
def train_inverse_pinn_gpu(noise_level=0.0, max_epochs=2500, lr=3e-3, log_every=250):
    ...
    for ts in obs_times:
        ...
        for ix, iy in zip(idx_x, idx_y):
            ...
            x_obs.append(x_sim[ix].item())
            y_obs.append(y_sim[iy].item())
            t_obs.append(ts)
            u_obs.append(u_val)
    ...
    model.eval()
    with torch.no_grad():
        x_eval = torch.tensor(X_sim.cpu().numpy().flatten(), dtype=torch.float32, device=device).view(-1, 1)
        y_eval = torch.tensor(Y_sim.cpu().numpy().flatten(), dtype=torch.float32, device=device).view(-1, 1)
...
# 3. RUN EXPERIMENTS
# -----------------------------------------------------------------------------
print("\n[PHASE 2] Clean Ground-Truth Parameter Discovery (0% Noise)...")
...
print("\n[PHASE 3] Noise Sensitivity Analysis (1%, 2%, 5%, 10% Noise)...")
...
# -----------------------------------------------------------------------------
# 4. SCIENTIFIC PLOTTING DIRECTLY FROM REAL EXPERIMENTAL DATA
# -----------------------------------------------------------------------------
print("\n[PHASE 4] Rendering Scientific Figures from Real Experimental Data...")
