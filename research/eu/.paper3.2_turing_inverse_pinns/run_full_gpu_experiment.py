#!/usr/bin/env python3
"""
run_full_gpu_experiment.py
==========================
End-to-End 100% GPU-native execution of the Turing Inverse PINN benchmark:
1. Forward Solver: 2D Schnakenberg PDE on GPU via RK4 + FDM (Neumann BC).
   Exact Thesis Ground-Truth (Pereira 2019, Table 27):
   a = 0.1305, b = 0.7695, kappa = 100.0, Du = 0.05, Dv = 1.00
2. Inverse PINN: 100% on GPU (RTX 3050).
   - Solves for (Du, Dv, kappa) from sparse observations of activator u ONLY.
   - Reconstructs unobserved hidden state v(x,y,t).
3. Noise Robustness Study: 0%, 1%, 2%, 5%, 10% Gaussian Noise.
4. Scientific Plotting: Generates Figures 1, 2, 3, and 4 from real training logs.
"""
...
# Restrict CPU threads so CPU stays cool and idle (<5% CPU)
...
# -----------------------------------------------------------------------------
# 1. FORWARD SIMULATION (100% ON CUDA GPU)
# -----------------------------------------------------------------------------
...
# Initial Condition (Gaussian Perturbation from Eq. 2.31)
u_gpu = u_star + 1e-3 * torch.exp(-100.0 * ((X_gpu - 1.0/3.0)**2 + (Y_gpu - 0.5)**2))
v_gpu = torch.full_like(u_gpu, v_star)

# 5-point Laplacian with zero-flux Neumann boundary conditions on GPU
...
print(f"  --> Forward simulation completed in {time.time()-t_start_fwd:.2f} s on GPU.")

# Save forward solution for verification
np.savez(
...
class InversePINN(nn.Module):
    def __init__(self, hidden_dim=64, num_layers=4):
        ...
        # Trainable log-parameters with uncalibrated initial guesses:
        # Du: real 0.05 -> initial 0.15 (+200%)
        # Dv: real 1.00 -> initial 0.50 (-50%)
        # kappa: real 100.0 -> initial 60.0 (-40%)
...
def train_inverse_pinn_gpu(noise_level=0.0, max_epochs=5000, lr=3e-3, log_every=500):
    print("\n" + "-"*82)
    print(f"  STARTING INVERSE PINN TRAINING ON GPU (Noise: {noise_level*100:.1f}%, Epochs: {max_epochs})")
    print("-"*82)
    ...
    # Evaluation on full 2D spatial grid at t = 2.0 s
    model.eval()
    with torch.no_grad():
...
res_clean = train_inverse_pinn_gpu(noise_level=0.0, max_epochs=5000, lr=3e-3, log_every=500)
...
for nl in noise_levels:
    res_nl = train_inverse_pinn_gpu(noise_level=nl, max_epochs=3500, lr=3e-3, log_every=1000)
    noise_results.append(res_nl)

# -----------------------------------------------------------------------------
# 4. SCIENTIFIC PLOTTING DIRECTLY FROM REAL DATA
# -----------------------------------------------------------------------------
print("\n[PHASE 4] Rendering Publication Figures...")
