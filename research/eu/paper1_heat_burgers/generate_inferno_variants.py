...
# Teste 1: Inferno no 3D e Inferno no 2D com range compartilhado e erro em Reds
# Teste 2: Inferno no 3D e Reds suave (tipo heat_comp_t10.png) no 2D
for var_idx, (cmp_cmap, err_cmap, name) in enumerate([
    ('inferno', 'Reds', 'variant1_inferno_reds'),
    ('Reds', 'Reds', 'variant2_reds_reds'),
    ('inferno', 'inferno', 'variant3_all_inferno')
]):
    ...
    im_d = ax_d.imshow(u_p, extent=[0, 1, 0, 1], origin='lower', cmap=cmp_cmap, vmin=vmin_val, vmax=vmax_val)
    ...
    im_e = ax_e.imshow(u_e, extent=[0, 1, 0, 1], origin='lower', cmap=cmp_cmap, vmin=vmin_val, vmax=vmax_val)
    ...
    im_f = ax_f.imshow(err, extent=[0, 1, 0, 1], origin='lower', cmap=err_cmap, vmin=0.0, vmax=err.max())
    ...
    fig.savefig(f'paper1_heat_burgers/{name}.png', dpi=350, bbox_inches='tight')
    plt.close(fig)

print("Generated variants successfully!")
