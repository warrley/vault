...
# Criando colormaps vermelhos homogêneos sem branco (de vermelho claro/salmão a vermelho escuro)
# 1. Truncated Reds de 0.25 a 1.0 (evita o branco puro de 0.0)
reds_no_white = matplotlib.colors.LinearSegmentedColormap.from_list(
    'reds_no_white', plt.cm.Reds(np.linspace(0.25, 1.0, 256))
)
# 2. Estilo exato de heat_comp_t10 (Reds suave)
reds_heat_comp = matplotlib.colors.LinearSegmentedColormap.from_list(
    'reds_heat_comp', plt.cm.Reds(np.linspace(0.30, 0.95, 256))
)
# 3. YlOrRd_r / OrRd
orrd_no_white = matplotlib.colors.LinearSegmentedColormap.from_list(
    'orrd_no_white', plt.cm.OrRd(np.linspace(0.25, 1.0, 256))
)

colormaps = [
    (reds_no_white, reds_no_white, 'preview_reds_homog1'),
    (reds_heat_comp, reds_no_white, 'preview_reds_homog2'),
    (orrd_no_white, reds_no_white, 'preview_orrd_homog3')
]

for cmp_cm, err_cm, fname in colormaps:
    ...
    # Linha 2: 2D sem branco
    vmin_val = min(u_p.min(), u_e.min())
    vmax_val = max(u_p.max(), u_e.max())

    # (d) PINN
    ax_d = fig.add_subplot(gs[1, 0])
    im_d = ax_d.imshow(u_p, extent=[0, 1, 0, 1], origin='lower', cmap=cmp_cm, vmin=vmin_val, vmax=vmax_val)
    ...
    im_e = ax_e.imshow(u_e, extent=[0, 1, 0, 1], origin='lower', cmap=cmp_cm, vmin=vmin_val, vmax=vmax_val)
    ...
    im_f = ax_f.imshow(err, extent=[0, 1, 0, 1], origin='lower', cmap=err_cm, vmin=0.0, vmax=err.max())
    ...
    fig.savefig(f'paper1_heat_burgers/{fname}.png', dpi=350, bbox_inches='tight')
    plt.close(fig)

print("Generated red range previews successfully!")
